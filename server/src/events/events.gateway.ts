...
interface Player {
  ...
  name: string;
  isReady: boolean;
  joinedAt: number; // For host migration logic
}
...
export class EventsGateway implements OnGatewayDisconnect {
  ...
  private readonly ROOM_TTL = 5 * 60 * 1000; // 5 minutes in milliseconds
    ...
    for (const [roomId, room] of this.rooms.entries()) {
      ...
      if (playerIndex > -1) {
        ...
        if (room.hostId === client.id) {
          ...
          const newHost = room.players.sort((a, b) => a.joinedAt - b.joinedAt)[0];
          room.hostId = newHost.id;
          console.log(`Host disconnected. New host in room ${roomId} is ${newHost.name}`);
        }
        break;
      }
    }
  ...
  handleCreateRoom(
    ...
    console.log(`Player ${name} (${client.id}) created and joined room ${roomId}`);
  }
  ...
  handleSetReady(
    ...
    if (room) {
      ...
      if (player) {
        ...
        this.broadcastRoomUpdate(roomId);
        console.log(`Player ${player.name} in room ${roomId} set ready status to ${isReady}`);
      }
    }
  }
}
